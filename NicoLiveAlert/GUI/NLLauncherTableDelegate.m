...
#import "NLLauncherTableDelegate.h"
#import "NicoLiveAlertDefinitions.h"

@implementation NLLauncherTableDelegate
...
- (id) initWithAryController:(NSArrayController	*) controller
{
	...
	return self;
}// end - (id) initWithAryController:(NSArrayController	*) controller
...
- (void) animationEffectDidEnd:(void *)contextInfo
{
	...
	if ([pbItems count] == 1)
	{
		NSData *data = [[pbItems lastObject] dataForType:LauncherPasteboardType];
		...
		[array removeObjectsAtArrangedObjectIndexes:indexes];
	}
...
{
#ifdef TRACECALL
	NSLog(@"tableView:acceptDrop:row:dorpOperation: : LauncherArrayController");
#endif
	BOOL dropped = NO;
	...
	for (NSPasteboard *item in pasteboardItems)
	{
		...
		for (NSString *type in types)
		{
			if ([type isEqualToString:LauncherPasteboardType] == YES)
			{
				...
				NSUInteger i = 0;
				for (NSDictionary *tmpdict in [array arrangedObjects])
				{
					if ([[tmpdict objectForKey:@"path"] isEqualToString:[dict objectForKey:@"path"]])
					{// drag move
						if (i < row)
							row --;
				...
				[array insertObject:dict atArrangedObjectIndex:row++];
				dropped = YES;
			}// end if
...
- (BOOL) tableView:(NSTableView *)aTableView writeRowsWithIndexes:(NSIndexSet *)rowIndexes toPasteboard:(NSPasteboard *)pboard
{
	...
	[pboard setData:pbData forType:LauncherPasteboardType];
